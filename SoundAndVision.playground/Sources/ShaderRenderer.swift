...
extension ShaderRenderer: MTKViewDelegate {
  ...
  public func draw(in view: MTKView) {
    ...
    commandEncoder?.setBytes(&frame, length: MemoryLayout<Float>.size * 1, index: 1)
    commandEncoder?.setBytes(&touchedPoint, length: MemoryLayout<CGPoint>.stride * touchedPoint.count, index: 2)
    commandEncoder?.setBytes(&numberOfTouches, length: MemoryLayout<Int>.size * 1, index: 3)
